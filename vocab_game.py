...
# 1. กำหนดค่าเริ่มต้นใน session_state ถ้ายินไม่มี (จุดที่ 1)
...
# 📌 ฟังก์ชันเคลียร์ค่าเมื่อกดปุ่มเริ่มต้นใหม่ (จุดที่ 2)
def reset_game():
...
# 3. ช่องรับคำตอบ (ใช้ value ผูกกับตัวแปรตรงๆ เพื่อสั่งเคลียร์ได้) (จุดที่ 6)
ans1 = st.text_input(
...
ans4 = st.text_input(
    "ข้อ 4: We write on paper using a `p _ n c _ l`. ✏️",
...
st.divider()
st.write("นาย จารุภัทร อรุณสิทธิ์ เลขที่ 9 ม.4/6")        st.session_state.is_ended = True
        st.rerun()
